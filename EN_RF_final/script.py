...
for run_id, outter_seed in enumerate(outter_seeds, start=1):
    ...
    for outter_fold,(outter_train_idx, outter_test_idx) in enumerate(outter_cv.split(X, y_encoded), start =1):
        ...
        for params in param_grid:
            ...
            for inner_train_idx, inner_val_idx in inner_cv.split(x_outter_train, y_outter_train):
                ...
                # ----------------  STEP 3: TRAIN RF ON TOP K FEATURES ----------------#
        ...
        # ----------------  RF ON FULL OUTTER TRAIN / TEST ON OUTTER TEST ----------------#
        ...
        final_rf.fit(x_outter_train_selected, y_outter_train)

        y_outter_pred = final_rf.predict(x_outter_test_selected)
        ...
        pred_df.to_csv(f'run{run_id}_outter_fold_{outter_fold}_predictions_ENLR_RF.csv', index=False)
        best_importance_df.to_csv(f'run{run_id}_outter_fold_{outter_fold}_feature_ranking_ENLR_RF.csv', index=False)
        coef_df.to_csv(f'run{run_id}_outter_fold_{outter_fold}_all_coefficients_ENLR_RF.csv')
        coef_selected_df.to_csv(f'run{run_id}_outter_fold_{outter_fold}_selected_coefficients_ENLR_RF.csv')
        cm_df.to_csv(f'run{run_id}_outter_fold_{outter_fold}_confusion_matrix_ENLR_RF.csv')
        report_df.to_csv(f'run{run_id}_outter_fold_{outter_fold}_classification_report_ENLR_RF.csv')
    ...
    outter_results_df.to_csv(f'run{run_id}_nested_cv_outter_results_ENLR_RF.csv', index=False)
    inner_results_df.to_csv(f'run{run_id}_nested_cv_inner_results_ENLR_RF.csv', index=False)
...
all_runs_df.to_csv('all_runs_summary_ENLR_RF.csv', index=False)
all_outter_results_df.to_csv('all_outter_results_ENLR_RF.csv', index=False)
all_inner_results_df.to_csv('all_inner_results_ENLR_RF.csv', index=False)
