...
def _box(ax, x, y, w, h, fc, ec=None, lw=1.2, zorder=2, alpha=1.0, pad=0.008):
...
def _draw_column1(ax):
    ...
    _centered_text(
        ax, col_cx, y1 + bh - 0.025, "Raw aCGH Data", fontsize=13, fontweight="bold"
    )
    _centered_text(
        ax, col_cx, y1 + bh / 2 - 0.01, "100 samples x 2,834 regions", fontsize=10
    )
    _centered_text(ax, col_cx, y1 + 0.02, "Discrete CN: {-1, 0, +1, +2}", fontsize=10)
    ...
    _centered_text(
        ax, col_cx, y2 + bh - 0.025, "Region Merging", fontsize=13, fontweight="bold"
    )
    _centered_text(
        ax, col_cx, y2 + bh / 2 - 0.01, "Pearson r > 0.8 threshold", fontsize=10
    )
    _centered_text(ax, col_cx, y2 + 0.02, "Label-free (no target leakage)", fontsize=10)
    ...
    _centered_text(
        ax, col_cx, y3 + bh - 0.025, "Merged Data", fontsize=13, fontweight="bold"
    )
    _centered_text(
        ax, col_cx, y3 + bh / 2 - 0.01, "100 samples x 273 segments", fontsize=10
    )
    _centered_text(
        ax, col_cx, y3 + 0.02, "Input to nested CV", fontsize=10, fontstyle="italic"
    )

    # Arrow from merged data to column 2.
    _arrow(
        ax, (col_r + 0.005, y3 + bh / 2), (0.27, y3 + bh / 2), lw=2.0, color=C["text"]
    )
...
def _draw_column2(ax):
    ...
    _centered_text(ax, col_cx, 0.895, "200 repeats (seeds 1001 - 1200)", fontsize=11)
    ...
    _centered_text(
        ax, col_cx, 0.55, "Within each outer fold:", fontsize=11, fontweight="bold"
    )
    ...
    train_bh = 0.12
    _box(
        ax, train_bx, train_by, train_bw, train_bh, fc=C["train"], ec="#8BAEC0", lw=1.0
    )
    train_cx = train_bx + train_bw / 2
    _centered_text(
        ax,
        train_cx,
        train_by + train_bh - 0.025,
        "80% Train",
        fontsize=11,
        fontweight="bold",
        color=C["text"],
    ...
    test_bw = bw / 2 - 0.04
    test_bh = 0.12
    _box(ax, test_bx, test_by, test_bw, test_bh, fc=C["test"], ec="#D08080", lw=1.0)
    test_cx = test_bx + test_bw / 2
    _centered_text(
        ax,
        test_cx,
        test_by + test_bh - 0.025,
        "20% Test",
        fontsize=11,
        fontweight="bold",
        color=C["text"],
    ...
    _arrow(
        ax, (train_cx, train_by), (inner_cx, inner_y + inner_h + 0.005), **ARROW_KW_THIN
    )
    ...
    _centered_text(ax, metric_cx, 0.25, "Stage 2 BA (BA2)", fontsize=9, color=C["text"])
    ...
    _centered_text(
        ax, col_cx, 0.085, "Report: mean +/- std", fontsize=11, fontstyle="italic"
    )
...
def _draw_column3(ax):
    ...
    _centered_text(
        ax, col_cx, inp_y + inp_h / 2, "Test samples", fontsize=11, fontweight="bold"
    )
    ...
    s1_x = col_l + 0.005
    s1_y = 0.70
    _box(ax, s1_x, s1_y, s1_w, s1_h, fc=C["stage1"], ec=C["stage1_dark"], lw=1.5)
    s1_cx = s1_x + s1_w / 2
    _centered_text(
        ax,
        s1_cx,
    ...
    _box(ax, her2_box_x, her2_y, her2_w, her2_h, fc="#FADBD8", ec=C["HER2"], lw=1.0)
    _centered_text(
        ax,
        left_cx,
        her2_y + her2_h / 2,
    ...
    _arrow(
        ax,
        (col_cx - 0.035, split_y),
        (left_cx, her2_y + her2_h + 0.003),
    ...
    s2_w = bw - 0.01
    s2_x = col_l + 0.005
    _box(ax, s2_x, s2_y, s2_w, s2_h, fc=C["stage2"], ec=C["stage2_dark"], lw=1.5)
    s2_cx = s2_x + s2_w / 2
    _centered_text(
        ax,
        s2_cx,
    ...
    _arrow(
        ax,
        (s2_cx + 0.035, s2_y),
        (tn_x, out_y + out_h + 0.003),
        connectionstyle="arc3,rad=-0.1",
        color=C["text"],
    )

    # HR+ box.
    _box(ax, hr_x - out_w / 2, out_y, out_w, out_h, fc="#D6E4F0", ec=C["HR"], lw=1.0)
    _centered_text(
        ax,
        hr_x,
        out_y + out_h / 2,
    ...
    # TN box.
    _box(ax, tn_x - out_w / 2, out_y, out_w, out_h, fc="#D5F0EB", ec=C["TN"], lw=1.0)
    _centered_text(
        ax,
        tn_x,
        out_y + out_h / 2,
    ...
    _box(ax, final_x, final_y, final_w, final_h, fc="#F8F8F8", ec=C["border"], lw=1.5)
    _centered_text(
        ax,
        col_cx,
        final_y + final_h / 2,
    ...
    _arrow(
        ax,
        (her2_box_x - 0.0097, her2_y + her2_h / 2),
    ...
    _arrow(
        ax,
        (hr_x, out_y),
        (col_cx - 0.02, final_y + final_h + 0.003),
...
def _draw_dividers(ax):
    ...
    for x_div in [0.26, 0.645]:
        ax.plot([x_div, x_div], [0.02, 0.96], color="#CCCCCC", lw=0.8, ls=":", zorder=0)
...
def _draw_panel_labels(ax):
    ...
    for x, y, lbl in labels:
        ax.text(
            ...
            va="top",
            zorder=10,
            bbox=dict(facecolor="white", edgecolor="none", alpha=1.0, pad=2.0),
        )
