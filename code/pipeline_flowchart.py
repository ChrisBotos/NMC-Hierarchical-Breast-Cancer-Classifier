...
def draw_arrow(
    ax, start, end, color="#555555", lw=1.5, style="->", connectionstyle="arc3,rad=0"
):
...
def build_flowchart():
    ...
    ax.text(
        MID,
        18.6,
        "Breast Cancer Subtype Classification from aCGH",
        ha="center",
        va="center",
        fontsize=13,
        fontweight="bold",
    ...
    draw_box(ax, (8.5, y), 2.0, bh, "Gene Map", C_DATA, text_lines=["hg18 coordinates"])
    ...
    draw_box(
        ...
        9.0,
        1.05,
        "Region Merging (CGHregions-style)",
        C_PREPROC,
        fontsize=10,
        text_lines=[
    ...
    # =================================================================
    y_p2 = 13.4
    phase_label(ax, MID, y_p2, "PHASE 2: MODEL TRAINING & EVALUATION", C_MODEL)
    ...
    draw_arrow(ax, (MID, y_p2 - 0.15), (MID, outer_y + outer_h - 0.45), C_ARROW)
    ...
    ax.text(
        MID,
        11.95,
        "Feature Selection (on inner training folds only)",
        ha="center",
        fontsize=8,
        fontweight="bold",
        color=C_FEAT,
    ...
    ax.text(
        MID,
        y_table,
        "2x2 AUROC Table  +  Confusion Matrices  +  " "Statistical Comparison",
        ha="center",
        va="center",
        fontsize=8,
        fontweight="bold",
    ...
    for x, (name, desc) in zip(x_positions, outputs):
        draw_box(
            ax, (x, y_out), out_w, 0.55, name, C_OUTPUT, fontsize=8.5, text_lines=[desc]
        )

    for x in x_positions:
        draw_arrow(ax, (MID, y_best), (x + out_w / 2, y_out + 0.55), C_ARROW)
