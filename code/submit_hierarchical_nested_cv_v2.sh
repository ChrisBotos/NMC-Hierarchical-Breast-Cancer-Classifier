#!/bin/bash
# Self-submitting SLURM wrapper for hierarchical_nested_cv_v2_runner.py.
#
...
#
# Usage:
#     bash code/submit_hierarchical_nested_cv_v2.sh                                       # default_run, config_files/server_v2.yaml
#     bash code/submit_hierarchical_nested_cv_v2.sh my_experiment                         # custom run name, server_v2 config
#     bash code/submit_hierarchical_nested_cv_v2.sh my_experiment config_files/local_v2.yaml  # custom run name + local config
#
# Array mapping:
...
if [[ -n "${SLURM_ARRAY_TASK_ID:-}" ]]; then
    ...
    PYTHON_CMD=(python3 "$PROJECT_DIR/code/hierarchical_nested_cv_v2_runner.py" \
        --pipeline "$PIPELINE" \
        --repeat "$REPEAT" \
        --config "$CONFIG_FILE" \
        --name "$RUN_NAME")
...
CONFIG_FILE="${2:-config_files/server_v2.yaml}"
...
RUNNER_SCRIPT="$PROJECT_DIR/code/hierarchical_nested_cv_v2_runner.py"
...
config['submit_hierarchical_nested_cv_v2'] = {
    'timestamp': '$TIMESTAMP',
    'run_name': '$RUN_NAME',
...
echo "  Stage 1:     kw_rf k=5 (fixed, threshold calibrated)"
echo "  Stage 2:     ${PIPELINES[*]} (7 pipelines)"
